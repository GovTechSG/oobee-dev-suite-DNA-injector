...
import { isKnownLowercaseTag } from './elements.js';
...
function computeJsProtectedRanges(code, from = 0, to = code.length) {
    const ranges = [];
    let i = from;
    while (i < to) {
        const c = code[i];
...
function computeHtmlProtectedRanges(code, from = 0, to = code.length) {
    const ranges = [];
    const patterns = [
        /<!--[\s\S]*?-->/g,
...
function computeLitProtectedRanges(code, from, to) {
    // Inside a Lit html`` template, expression holes `${...}` are JS and can
    // contain nested templates already handled by the framework's own zones.
    // But we still want to protect ${...} chunks from being scanned as HTML.
    const ranges = [];
    let i = from;
    while (i < to) {
        if (code[i] === '$' && code[i + 1] === '{') {
...
function findAngularTemplateZones(code) {
    ...
    while ((d = decoratorRe.exec(code)) !== null) {
        ...
        // Match template: `...`, template: '...', or template: "..."
...
function findLitTemplateZones(code) {
    // Tagged template literals: html`...`, svg`...`, render`...`, plus
    // common lit-labs helpers static.html and unsafeHTML style patterns.
    const zones = [];
...
function injectDNA(code, filePath, options = {}) {
    ...
    for (const zone of zones) {
        ...
        while ((m = tagRegex.exec(code)) !== null) {
            ...
            if (isLower) {
                // Whitelist: only known HTML/SVG/MathML/framework-builtin
                // tags or hyphenated custom elements.
                if (!isKnownLowercaseTag(tagName, framework)) continue;
            } else {
                // Capital-letter components: must not look like a TS generic.
